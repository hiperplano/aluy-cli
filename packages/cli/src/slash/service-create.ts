...
export function buildServiceCreateSystemPrompt(descricao: string): string {
  ...
  return [
    'Você é um especialista em SERVIÇOS plugáveis do aluy. Sua tarefa é',
    ...
    '    expediente, `HH:MM`) — sem os dois o runner não sabe operar.',
    '  - `channel:` — OPCIONAL: para onde reporta e pergunta LONGE da máquina (ex.:',
    '    `telegram:<chat-id>`). NÃO insista nem invente um: sem canal, o serviço',
    '    espera em silêncio e o dono responde com `aluy service attach`. Só declare',
    '    se o dono JÁ disse que quer ser avisado fora do terminal.',
    '  - `autonomy:` — `ask` (default; pergunta antes de agir) ou `yolo-scoped`',
    '    (turno AUTÔNOMO mas CONFINADO: o serviço não pergunta — não há dono no turno',
    '    para responder — mas a cerca de workspace/path-deny/anti-SSRF permanecem',
    '    intactas). Qualquer outro valor é rejeitado pelo parser. NUNCA proponha',
    '    `yolo-scoped` por conta própria — só escreva se o dono EXPLICITAMENTE pediu',
    '    um serviço que roda sem aprovação; na dúvida, o default `ask` é o seguro.',
    ...
    '  - HÁ um funil de decisão? (a regra de ouro: "N estudos, nenhum',
    ...
    '   channel: <conector>:<alvo>  # OPCIONAL — ex.: telegram:123456. OMITA a linha',
    '                               # inteira se o dono não pediu aviso remoto.',
    ...
    '   opcional + corpo = system prompt da persona). `tools:` (quando presente)',
    '   ACEITA DUAS formas EQUIVALENTES — use a que for mais natural, nunca deixe',
    '   a chave presente mas vazia (isso é rejeitado, fail-closed):',
    '   ```',
    '   tools: read_file, grep          # 1) uma linha, separado por vírgula',
    '   ```',
    '   ```',
    '   tools:                          # 2) lista YAML em bloco',
    '     - read_file',
    '     - grep',
    '   ```',
    '   **TOPOLOGIA DE FUNIL, quando',
    '   houver decisor de risco/execução:**',
    ...
    '     ferramenta. Não pule este ponto: é o núcleo do que dá segurança ao serviço.',
    '',
    ...
    '   atividade do turno deve ser o FECHAMENTO/reporte ("um turno que não reportou',
    '   é um workflow que não terminou"). Se o dono descreveu uma ROTINA de revisão',
    '   periódica (retro), crie também `workflows/retro.md` com um `schedule:`',
    '   PRÓPRIO no frontmatter dele (ex.: sexta 18h).',
    '',
    '4. **`rules.md`** (opcional) — regras de JULGAMENTO em prosa (ex.: "prefira',
    '   sair antes de payroll"). ATENÇÃO: se algo que o',
    ...
    '   plugado depois"). Se a skill tem teto de execução (ex.: "no',
    '   máximo N contratos"), documente no `SKILL.md` que o teto vive DENTRO do',
    '   script — não como algo que o modelo decide.',
    '',
    ...
    '',
    '## CRIAR NÃO É LIGAR (não pule este final)',
    '',
