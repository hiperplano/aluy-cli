...
- 🧬 O caminho é **fire-and-forget**: o turno já acabou quando a chamada sai, e falha, timeout ou resposta suja caem de volta nas sete frases — ornamento não derruba nem atrasa nada. A sugestão pendente é **abortada** quando um turno novo começa, para não pintar por cima de um contexto morto.

### Segurança

- 🧹 **`sanitizeSuggestion` — ponto único e obrigatório.** Com o menu fechado, a sugestão era inerte; com texto livre do modelo, o que ele devolver chega ao **composer do dono**. O saneamento dobra qualquer resposta em uma linha usável: primeira linha não-vazia, cerca de código removida, marcador de lista e aspas envolventes fora, teto de 120 caracteres. Ele **não julga a intenção** do texto — não temos classificador para isso e fingir que temos seria pior. A garantia é outra, e foi **verificada no código**: a TUI só sugere com o composer **e** a fila de type-ahead vazios, e o Tab apenas preenche — o Enter continua sendo do dono, então não há caminho de auto-submissão. O caller é o mesmo do `/ask`: **sem tools**, read-only por construção.
...
- 📢 **O `--help` negava duas funcionalidades que estão prontas.** O texto anunciava o conector Telegram como "preparação — a bridge ainda NÃO está ativa" e os serviços como "fase 1: fundação SEM runner ainda", com `create/start/stop/attach` "chegam na fase 2". O código desmente os dois desde a rc.166: a bridge sobe no boot com `aluy --telegram` (long-poll do chat allowlistado + tool `telegram_send`, dormente sem token e com zero egress), e o runner existe (`service/runner.ts`) com `start/stop/logs/attach` já no parser. Pior que a mentira: o `--help` era o ÚNICO lugar onde a flag `--telegram` podia ser descoberta, e ela não estava na lista de opções — quem lesse a ajuda inteira concluiria que não dá para ligar. Os dois blocos foram reescritos pelo que o comando faz HOJE, os quatro subcomandos que faltavam entraram com suas flags reais (`--group`, `--yes`, `-f/--follow`, `-n <N>`), e `aluy telegram` ganhou linha no bloco de uso.
- 🔐 **O `--help` prometia que a credencial fica "SÓ no keychain do SO — nunca em texto em claro".** A primeira metade é falsa desde a emenda da rc.135: existe um terceiro lugar legítimo, o cofre em arquivo CIFRADO (`~/.aluy/credentials.enc`, AES-256-GCM com chave derivada da máquina), e desde 31/08 ele vale também para o token de conector — o `telegram login` dizia "nunca em arquivo" enquanto gravava exatamente num arquivo. Em servidor headless sem Secret Service, o keychain do kernel é MEMÓRIA e o cofre em arquivo é a única persistência real, então o texto antigo descrevia o oposto do que acontece na máquina de quem mais depende dele. O `telegram logout` também passou a dizer que apaga dos DOIS.
- 🧹 Comentários que repetiam "fase 1 — sem runner ainda" em `cli.ts`, `bin/aluy.ts` e no cabeçalho do teste de `service` (o próprio `commands/service.ts` já estava correto), e o comentário do `/telegram` em `slash/commands.ts` que dizia "a bridge em si segue inerte".
- 🤖 **O MCP de RPA oferecido no onboard não instalava.** O catálogo curado oferecia "RPA (Aluy)" com `uvx aluy-mcp-rpa`, e esse pacote **não existe no PyPI** (medido: `/pypi/aluy-mcp-rpa/json` ⇒ 404). Quem aceitava a oferta recebia um erro no PRIMEIRO contato com o produto — no passo de instalação, que é onde a confiança é mais barata de perder. O teste do catálogo passava porque conferia a FORMA (command/args não-vazios, `npx` ou `uvx`), nunca o ALVO: bem-formado não é instalável. Agora o comando resolve o projeto direto do repositório (`uvx --from git+https://github.com/hiperplano/aluy-mcp-rpa aluy-mcp-rpa`) — o console-script `aluy-mcp-rpa` existe no `[project.scripts]` de lá, então o nome não muda. Um teste novo trava a origem (verificado por mutação: voltar ao nome puro reprova). Quando o pacote for publicado, basta remover o `--from`.
### Alterado
...
- 🔇 **Vinte tentativas em silêncio, e o dono só via `Λ aluy` vazios se empilhando:** com um provider recusando (medido contra um `429` real), a tela enchia de blocos de fala em branco — quatro em 24 segundos, um a cada espera de 5s, até vinte — e NENHUMA palavra sobre o motivo. Duas causas somadas. (1) A retentativa mora DENTRO do caller (`streaming-caller`, laço `decideRetry`), e cada tentativa chama `sink.onStart`, que empurra um bloco `aluy` novo; como o controller não era avisado, os anteriores ficavam ÓRFÃOS (`streaming: true`, texto vazio) e iam parar até no arquivo de sessão. (2) O `onRetry` existia em `StreamingModelCallerOptions`, era DISPARADO a cada tentativa — e ninguém o ligava: a razão da falha (`HTTP 429`, transporte, 5xx) morria ali dentro. É a MESMA classe do `upstreamByModel` (rc.137) e do canal de raciocínio acima: peça pronta, ponta solta. Agora o aviso chega ao controller, que DESCARTA o turno em voo que ficou vazio (fala parcial é preservada — só o mudo se descarta) e mostra uma caixa VIVA com o motivo, a contagem `n/N` e como cancelar. Um provider que recusa continua recusando; o que muda é o dono saber disso na primeira tela em vez de encarar blocos em branco.
